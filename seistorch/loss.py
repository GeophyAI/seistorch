...
from seistorch.signal import batch_sta_lta, travel_time_diff, pick_first_arrivals, normalize_trace_max
# import geomloss
from seistorch.utils import interp1d
...
class FATT(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        np.save("/public1/home/wangsw/FWI/NO_LOWFREQ/fatt/masked_obs.npy", masked_obs.cpu().detach().numpy())
        np.save("/public1/home/wangsw/FWI/NO_LOWFREQ/fatt/masked_syn.npy", masked_syn.cpu().detach().numpy())

        # Normalize the observed and synthetic data
        masked_obs = normalize_trace_max(masked_obs)
        masked_syn = normalize_trace_max(masked_syn)
        ...
        np.save("/public1/home/wangsw/FWI/NO_LOWFREQ/fatt/adj.npy", adj.cpu().detach().numpy())
        ...
        for t in range(ntraces):
            for c in range(nchannles):
                ...
                if torch.max(torch.abs(_x))>1e-5 or torch.max(torch.abs(_y))>1e-5:
                    ...
                    logits = F.gumbel_softmax(cc*scale, tau=50, hard=True)
...
class SoftDTW(torch.nn.Module):
    ...
    def __init__(self, use_cuda=True, gamma=10, normalize=False, bandwidth=None, dist_func=None):
        """
