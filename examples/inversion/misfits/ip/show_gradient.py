...
dh = 10
bwidth = 50
ip = torch.load('results/ip/grad_vp_0.pt').cpu().numpy()
l2 = torch.load('results/l2/grad_vp_0.pt').cpu().numpy()

ip = ip[bwidth:-bwidth, bwidth:-bwidth]
...
vmin, vmax = np.percentile(ip, [1, 99])
kwargs = dict(vmin=vmin, vmax=vmax, cmap='gray', aspect='auto', extent=extent)
axes[1].imshow(ip, **kwargs)
axes[1].set_title('IP Gradient')
plt.colorbar(axes[1].imshow(ip, **kwargs), ax=axes[1], orientation='vertical')
...
ax.plot(x, ip[ip.shape[0]//2, :], label='IP')
