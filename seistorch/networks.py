...
class SirenScale(nn.Module):
    ...
    def __init__(self, 
        ...
        self.net = nn.Sequential(*self.net)
        self.coords = self.generate_mesh(self.scaled_shape, dh)
        self.load_pretrained(pretrained)
    ...
    def generate_mesh(self, mshape, dh):
        ...
        mgrid = mgrid.reshape(-1, len(mshape))
        # rand coords
        # mgrid = torch.rand_like(mgrid)*2-1
        return mgrid
    ...
    def forward(self, coords):
        coords = coords.clone().detach().requires_grad_(True) # allows to take derivative w.r.t. input
        output = self.net(coords).view(1, 1, *self.scaled_shape)
        print(output.shape)
