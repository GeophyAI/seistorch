...
def laplace_ps(u, h, lap):

    fft = torch.fft.fft2(u, dim=[2, 3])

    # apply laplace operator
    fft = fft * lap.unsqueeze(0).unsqueeze(0)
    
    return torch.fft.ifft2(fft, dim=[2, 3]).real

def laplace_fd(u, h):
    return torch.nn.functional.conv2d(u, kernel, padding=1) / (h ** 2)

def step_ps(u_pre, u_now, c=1.5, dt=0.001, h=10./1000., b=None, lap=None):

    u_next = ((dt**-2 + b * dt**-1).pow(-1) *
                (2 / dt**2 * u_now - (dt**-2 - b * dt**-1) * u_pre
                + c.pow(2) * laplace_ps(u_now, h, lap)))
    return u_next

def step_fd(u_pre, u_now, c=1.5, dt=0.001, h=10./1000., b=None, lap=None):

    u_next = ((dt**-2 + b * dt**-1).pow(-1) *
                (2 / dt**2 * u_now - (dt**-2 - b * dt**-1) * u_pre
                + c.pow(2) * laplace_fd(u_now, h)))
    return u_next

def forward(wave, c, b, src_list, domain, dt, h, dev, recz=0, pmln=50, ps=True):
    nt = wave.shape[0]
    nz, nx = domain
    nshots = len(src_list)
    ...
    # wavenumber
    _, _, nx, ny = u_pre.shape
    kx = torch.fft.fftfreq(nx, h.item()).to(device)*(2*torch.pi)
    ky = torch.fft.fftfreq(ny, h.item()).to(device)*(2*torch.pi)

    # laplace operator
    k_x_grid, k_y_grid = torch.meshgrid(kx, ky, indexing='ij')
    lap = - (k_x_grid ** 2 + k_y_grid ** 2)
    ...
    for it in range(nt):
        ...
        u_next = step(u_pre, u_now, c, dt, h, b, lap)
