...
dtype = np.float32
nz, nx = 100, 200
vel = np.ones((nz, nx), dtype=dtype)*2500.
vel[88:,:] = 1500.
...
# smooth the rx and rz
rx_smooth = rx.copy()
rz_smooth = rz.copy()
for i in range(10):
    gaussian_filter(rx_smooth, sigma=1, output=rx_smooth)
    gaussian_filter(rz_smooth, sigma=1, output=rz_smooth)

plt.plot(rz[:,64], label="Original")
plt.plot(rz_smooth[:,64], label="Smoothed")
plt.legend()
plt.show()
...
src_x = np.linspace(49, 50, 1)
src_z = np.ones_like(src_x)*40
...
#            [0, 1, ..., 255], [5, 5, ..., 5],
#            ],
receiver_locx = np.arange(149, 150, 1)
receiver_locz = np.ones_like(receiver_locx)*40
...
np.save(os.path.join(vel_path, "zero_m.npy"), zero_m)
np.save(os.path.join(vel_path, "bg_vel.npy"), bg_vel)
np.save(os.path.join(vel_path, "rx_smooth.npy"), rx_smooth)
np.save(os.path.join(vel_path, "rz_smooth.npy"), rz_smooth)
