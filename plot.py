...
# d = np.load("/mnt/data/wangsw/inversion/elastic_marmousi/data/elastic_fixed.npy", allow_pickle=True)
...
# ax0=axes[0].imshow(dx.squeeze(), vmin=vmin, vmax=vmax, aspect='auto', cmap=plt.cm.seismic)
# vmin, vmax = np.percentile(dz, [2, 98])
# ax1=axes[1].imshow(dz.squeeze(), vmin=vmin, vmax=vmax, aspect='auto', cmap=plt.cm.seismic)
# plt.show()
# plt.colorbar(ax0)
...
ax0=axes[0].imshow(vp, vmin=vmin, vmax=vmax, aspect='auto', extent=extent, cmap=plt.cm.seismic)
